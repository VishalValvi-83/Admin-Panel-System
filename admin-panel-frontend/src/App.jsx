...
const App = () => {
  const { isAuthenticated, userRole, userId, loading, logout } = useContext(AuthContext);
  ...
  useEffect(() => {
    if (isAuthenticated) {
      setCurrentPage(userRole === 'ADMIN' ? 'admin-dashboard' : userRole === 'MANAGER' ? 'manager-dashboard' : 'user-dashboard');
    } else {
      setTimeout(() => {
